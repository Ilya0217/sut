...
/*
Назначение: Преобразование записи аудита в JSON
Входные данные: log — объект записи аудита
Выходные данные: JSON представление записи
*/
static json audit_log_to_json(const AuditLog& log)
{
    return {
        {"id", log.id},
...
/*
Назначение: Обработка GET /api/audit — список записей аудита с фильтрацией
Входные данные: query params: page, per_page, object_type, user_id
Выходные данные: JSON {items, total, pages, page}
*/
static void handle_list_audit(const httplib::Request& req, httplib::Response& res)
{
    ...
    for (const auto& log : result.items) {
        items.push_back(audit_log_to_json(log));
    }
    json_response(res, {
